...
if(isset($_POST)) {
    ...
    switch($_POST['current_table']) {
        ...
        case "boardgamebooking":
            $sql = "UPDATE boardgamebooking SET Status = '{$status}', ReceiptDate = '".date("Y-m-d")."', ReceiptTime = '".date("H:i:s")."' WHERE BookingID = $id";
            break;
    ...
    if(mysqli_affected_rows($conn) >= 1) {
        ...
        if($status == 4)
            $validator['messages'] = "成功取消活動 !";
        else
            $validator['messages'] = "成功更新活動狀態為完成 !";
