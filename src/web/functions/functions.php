	...
	if ($query) {
		...
		if($num_rows == 0) {
			...
			echo "<div id='event_row'><h4> Event ".$x." </h2>";
				echo "<h2><small> TITLE : </small>" . stripslashes(urldecode($row['event'])) . "</h2>";
				echo "<h2><small> From : </small>".$from_hr." ".$from_min."</h2>";
				echo "<h2><small> To : </small>".$to_hr." ".$to_min."</h2>";
				echo "<div class='content'><small> Description : </small>".stripslashes(urldecode($row['description']))."</div>";
			echo "</div>";
			}
		}
		echo "</div>";
